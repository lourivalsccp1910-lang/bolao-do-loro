# ROADMAP.md

# Bolão do Loro

## Roadmap Oficial

Este documento controla toda a evolução do sistema.

Nenhuma Sprint será iniciada sem planejamento.

Nenhuma versão será considerada concluída sem validação.

---

# STATUS GERAL

Projeto iniciado

Versão Atual:

Release 0.3

Sprint Atual:

Sprint 1

Status:

🟡 Em Validação

---

# ROADMAP

## Release 0.3

### Sprint 1

### Fundação do Sistema

Objetivo:

Construir a base definitiva do projeto.

### Entregas

- [x] Estrutura definitiva
- [x] Dashboard
- [x] Header
- [x] Sidebar
- [ ] Área Principal
- [ ] Layout Responsivo
- [x] Navegação
- [ ] Tema Visual
- [x] Sem erros de import/export
- [x] Projeto compilando

Resultado esperado

Abrir o navegador e visualizar o Dashboard do Bolão do Loro.

Status

🟡 Em validação

---

## Release 0.4

### Sprint 2

### Participantes

Objetivo

Criar o módulo completo de participantes.

Entregas

- [ ] Cadastro
- [ ] Alteração
- [ ] Exclusão
- [ ] Pesquisa
- [ ] Paginação
- [ ] Validação

Status

⚪ Não iniciado

---

## Release 0.5

### Sprint 3

### Campeonatos

Objetivo

Cadastrar todos os campeonatos.

Entregas

- [ ] Cadastro
- [ ] Alteração
- [ ] Exclusão
- [ ] Status
- [ ] Datas

Status

⚪ Não iniciado

---

## Release 0.6

### Sprint 4

### Jogos

Objetivo

Cadastrar jogos.

Entregas

- [ ] Cadastro
- [ ] Datas
- [ ] Rodadas
- [ ] Horários
- [ ] Resultados

Status

⚪ Não iniciado

---

## Release 0.7

### Sprint 5

### Palpites

Objetivo

Permitir lançamento dos palpites.

Entregas

- [ ] Cadastro
- [ ] Alteração
- [ ] Encerramento
- [ ] Validação

Status

⚪ Não iniciado

---

## Release 0.8

### Sprint 6

### Ranking

Objetivo

Criar classificação automática.

Entregas

- [ ] Pontuação
- [ ] Critérios de desempate
- [ ] Ranking Geral
- [ ] Ranking por campeonato

Status

⚪ Não iniciado

---

## Release 0.9

### Sprint 7

### Financeiro

Objetivo

Controlar pagamentos.

Entregas

- [ ] Recebimentos
- [ ] Pendências
- [ ] Caixa
- [ ] Relatórios

Status

⚪ Não iniciado

---

## Release 1.0

Sistema Operacional

Objetivo

Primeira versão completa.

Entregas

- [x] Dashboard
- [ ] Participantes
- [ ] Campeonatos
- [ ] Jogos
- [ ] Palpites
- [ ] Ranking
- [ ] Financeiro
- [ ] Relatórios
- [ ] Configurações

Status

⚪ Não iniciado

---

# Melhorias Futuras

## Versão 2.0

Aplicativo Mobile

- Android

- iPhone

---

## Versão 3.0

Integração com APIs esportivas.

---

## Versão 4.0

Integração completa com WhatsApp.

---

## Versão 5.0

Sistema SaaS.

Permitir que outros organizadores criem seus próprios bolões.

---

# Histórico

## Release 0.3

Início oficial do desenvolvimento.

Arquitetura definitiva.

Documentação criada.

PROJECT.md criado.

ROADMAP.md criado.

---

Última atualização:

27/08/2026